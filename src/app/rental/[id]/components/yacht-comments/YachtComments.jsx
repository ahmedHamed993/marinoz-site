...
import YachtComment from "./YachtComment";
...
const YachtComments = () => {
  ...
  console.log("comments",data?.rentals_by_pk?.rental_reviews);
  return (
    <div
      style={{
        display: "flex",
    ...
    >
      {data?.rentals_by_pk?.rental_reviews?.map((comment) => (
        <YachtComment key={comment.id} comment={comment} ratingValue={comment?.stars} />
      ))}
    </div>
  );
};
